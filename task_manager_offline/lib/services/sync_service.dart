...
class SyncService {
  ...
  Future<SyncResult> sync() async {
    ...
    // Check server reachability before attempting sync to avoid spammed socket errors
    final serverReachable = await _api.checkConnectivity();
    if (!serverReachable) {
      print('📡 Servidor não alcançável (health-check falhou) - operações mantidas na fila');
      return SyncResult(
        success: false,
        message: 'Servidor não alcançável',
      );
    }
    ...
    try {
      ...
      print('🔎 Api baseUrl = ${ApiService.baseUrl}');
  ...
  Future<int> _pushPendingOperations() async {
    ...
    for (final operation in operations) {
      print('▶️ Processando operação ${operation.id} (type=${operation.type})');
      try {
        await _processOperation(operation);
        await _db.removeSyncOperation(operation.id);
        print('✅ Operação ${operation.id} processada e removida da fila');
        successCount++;
      } catch (e) {
  ...
  Future<void> _processOperation(SyncOperation operation) async {
    print('🔧 _processOperation -> ${operation.id}');
  ...
  Future<void> _pushCreate(SyncOperation operation) async {
    ...
    if (task == null) return;
    print('⬆️ _pushCreate -> enviando task ${task.id} para servidor');
    final serverTask = await _api.createTask(task);
    print('⬅️ _pushCreate -> resposta recebida, server version=${serverTask.version}');
    ...
    print('💾 _pushCreate -> tarefa ${task.id} atualizada localmente como synced');
  }

  Future<void> _pushUpdate(SyncOperation operation) async {
    final task = await _db.getTask(operation.taskId);
    if (task == null) return;
    print('⬆️ _pushUpdate -> atualizando task ${task.id} no servidor');
    try {
      ...
      if (result['conflict'] == true) {
        ...
        print('💾 _pushUpdate -> tarefa ${task.id} atualizada localmente como synced');
      }
    } catch (e) {
      // Se o servidor retornou 404 (não encontrado), tente criar a tarefa no servidor
      final err = e.toString();
      if (err.contains('404') || err.toLowerCase().contains('not found')) {
        print('⚠️ _pushUpdate -> tarefa não encontrada no servidor, tentando criar (POST)');
        final serverTask = await _api.createTask(task);
        await _db.upsertTask(
          task.copyWith(
            version: serverTask.version,
        ...
        print('💾 _pushUpdate -> tarefa ${task.id} criada no servidor e marcada como synced localmente');
      } else {
        rethrow;
      }
    }
